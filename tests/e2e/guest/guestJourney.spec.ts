...
test.describe('Guest User Journey', () => {
  test('TC-E2E-GUEST-001: Complete guest browsing flow', async ({ page, context }) => {
    ...
    // Verify guest session created (stored in fabricspeaks_session_id per cart.config.ts)
    const sessionId = await page.evaluate(() => localStorage.getItem('fabricspeaks_session_id'));
    expect(sessionId).toBeTruthy();
    expect(sessionId).toMatch(/^guest_[a-f0-9]{32}$/); // Matches SESSION_ID_PREFIX + 32 hex chars
